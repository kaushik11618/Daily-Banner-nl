...
const Post = ({ onLinkClick }) => {
    ...
    const showRequest = async () => {
      try {
        const request = await axios.get(
          "http://192.168.29.12:3000/api/admin/request",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleChange = async (userId, event) => {
    ...
    const response = await fetch(
      "http://192.168.29.12:3000/api/admin/request-status-change",
      {
        method: "PATCH",
        headers: {
                ...
                {user.map((user, index) => {
                  return (
                    <TableRow hover role="checkbox" tabIndex={-1}>
                      ...
                      <TableCell align="left">
                        <Typography variant="h5">{user.note}</Typography>
                      </TableCell>
                      <TableCell align="left">
                      ...
                      <TableCell align="left">
                        <Typography variant="h5" className="ms-3">
                          <Checkbox
                            checked={parseInt(user.social_media) === 1}
                          />
                        </Typography>
                      </TableCell>
                      <TableCell align="left">
                      ...
                      <TableCell align="left">
                        <Typography variant="h5" className="ms-3">
                          <Checkbox
                            checked={parseInt(user.phone_number) === 1}
                          />
                        </Typography>
                      </TableCell>
                      <TableCell align="left">
                      ...
                      <FormControl sx={{ m: 1, minWidth: 100, marginTop: 2 }}>
                        {user.post_status &&
                        (user.post_status.name === "Verified" ||
                          user.post_status.name === "Completed") ? (
